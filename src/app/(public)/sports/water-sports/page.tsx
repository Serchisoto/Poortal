        ...
        <div className="min-h-screen bg-white flex flex-col relative pb-10">
            <div className="flex items-center justify-between px-6 pt-6 pb-4 bg-white sticky top-0 z-10">
                ...
                <div className="border border-slate-200 bg-white rounded-full px-10 py-3 shadow-sm absolute left-1/2 -translate-x-1/2">
                    <h1 className="text-sm font-bold text-slate-800 tracking-wide uppercase">WATER SPORTS</h1>
                </div>
            </div>
            <main className="container mx-auto max-w-md px-6 flex flex-col items-center mt-2">
                ...
                <div className="w-full grid grid-cols-2 gap-3 pb-8">
                    {experiences.map((exp) => (
                        <Link href={`/experience/${exp.id}`} key={exp.id} className="aspect-[4/5] rounded-xl overflow-hidden bg-sky-900 relative shadow-sm active:scale-95 transition-transform">
                            {exp.cover_image_url ? (
                                <Image src={exp.cover_image_url} alt={exp.title} fill className="object-cover opacity-85" />
                            ) : (
                                <div className="absolute inset-0 bg-gradient-to-br from-cyan-600 via-sky-700 to-blue-800" />
                            )}
                    ...
                    {experiences.length === 0 && (
                        <div className="col-span-2 flex flex-col items-center py-16 text-center">
                            ...
                            <p className="text-sm font-semibold text-[#1b6d72]">No water sports available right now.</p>
                        </div>
                    )}
                </div>
            </main>
