...
export const metadata = {
    title: 'Health - POORTAL',
}

export default async function WellnessHealthPage() {
    const [category, subcategory] = await Promise.all([
        getCategoryBySlug('wellness'),
        getSubcategoryBySlug('health'),
    ])
            ...
            <main className="container mx-auto max-w-md px-6 flex flex-col items-center mt-2">
                ...
                <div className="w-full flex flex-col gap-4 pb-8">
                    {experiences.map((exp) => (
                        ...
                        >
                            <div className="h-36 bg-emerald-50 relative w-full overflow-hidden">
                                {exp.cover_image_url ? (
                                    ...
                                    <div className="flex h-full w-full items-center justify-center bg-emerald-50">
                                        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#1b6d72" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    {experiences.length === 0 && (
                        <div className="flex flex-col items-center py-16 text-center text-slate-500">
                            No health services available right now.
                        </div>
                    )}
                </div>
