...
export const metadata = {
    title: 'Daylife - POORTAL',
}

export default async function DaylifeSearchPage() {
    const [category, subcategory] = await Promise.all([
        getCategoryBySlug('party'),
        getSubcategoryBySlug('daylife'),
    ])
                ...
                <div className="w-full grid grid-cols-2 gap-3 pb-8">
                    {experiences.map((exp) => (
                        <Link
                            ...
                            key={exp.id}
                            className="aspect-[4/5] rounded-xl overflow-hidden bg-amber-50 relative shadow-sm active:scale-95 transition-transform"
                        >
                            {exp.cover_image_url ? (
                                <Image src={exp.cover_image_url} alt={exp.title} fill className="object-cover opacity-90" />
                            ) : (
                                <div className="absolute inset-0 bg-gradient-to-br from-amber-400 via-orange-300 to-yellow-200" />
                            )}
                            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-3">
                                ...
                                <span className="text-amber-300 font-bold text-xs mt-1">
                    ...
                    {experiences.length === 0 && (
                        <div className="col-span-2 py-16 text-center text-slate-500">
                            No daylife experiences available right now.
                        </div>
                    )}
                </div>
